...
import static cn.hzw.graffiti.GraffitiView.Pen.MOSAIC;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction() & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_CANCEL:
                ...
                invalidate();
                return true;
            case MotionEvent.ACTION_MOVE:           // TODO: 17/12/22
                // 单点滑动
                if (mTouchMode < 2) {
                    mLastTouchX = mTouchX;
                    mLastTouchY = mTouchY;
...
//            }
    ...
    private void initCanvas() {
        ...
        mGraffitiBitmap = mBitmap.copy(mBitmap.getConfig(), true);
    ...
    //切换马赛克模式
    private void switchMosaic() {       // TODO: 17/12/22
        if(bmCoverLayer == null){
            bmCoverLayer = Bitmap.createBitmap(mBitmap.getWidth(), mBitmap.getHeight(), Bitmap.Config.ARGB_4444);
            Bitmap mosaicBitmap = MosaicUtils.getMosaic(mBitmap);
            Canvas canvas = new Canvas(bmCoverLayer);
            canvas.drawBitmap(mosaicBitmap, 0, 0, null);
            canvas.save();
            mosaicBitmap.recycle();

            bmMosaicLayer = Bitmap.createBitmap(mBitmap.getWidth(), mBitmap.getHeight(), Bitmap.Config.ARGB_4444);
        }
        mShape = Shape.HAND_WRITE;
    }


    private void drawMosaicPath(Path path){
        bmTouchLayer = Bitmap.createBitmap(mBitmap.getWidth(), mBitmap.getHeight(),
        ...
        bmTouchLayer = null;
        mBitmapCanvas.drawBitmap(bmMosaicLayer, 0, 0, null);
        mBitmapCanvas.save();
    }
