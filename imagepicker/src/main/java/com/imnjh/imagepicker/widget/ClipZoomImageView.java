...
public class ClipZoomImageView extends AppCompatImageView implements
  ...
  public void onGlobalLayout() {
    if (once) {
      ...
      //如果图片宽高都比裁剪框小，默认拉伸宽度，自适应高度
      } else if (drawableW < frameSize && drawableH < frameSize) {
        float scaleW = 1.0f * frameSize / drawableW;
        float scaleH = 1.0f * frameSize / drawableH;
        scale = Math.max(scaleW, scaleH);
        mImageBorderView.setCropHeight((int) (scale * drawableH));
      }*/
      scale = 1.0f * frameSize / drawableW;
      scaleHeight = (int) (scale * drawableH);

      //如果图片缩放之后的图片还比默认裁剪高度小 就用图片的
      /*if(scaleHeight <= getHVerticalPadding()){
        mImageBorderView.setCropHeight(scaleHeight);
      }*/

      initScale = scale;
      SCALE_MID = initScale * 2;
      SCALE_MAX = initScale * 4;
      scaleMatrix.postTranslate((width - drawableW) / 2, (height - drawableH) / 2);
  ...
  public Bitmap clip() {
      ...
      int startY = getHVerticalPadding();
      if(cropHeight == 0){
          return Bitmap.createBitmap(bitmap, horizontalPadding, startY, getWidth() - 2 * horizontalPadding, getWidth() - 2 * horizontalPadding);
      }else{
          return Bitmap.createBitmap(bitmap, horizontalPadding, startY, getWidth() - 2 * horizontalPadding, cropHeight - 2 * horizontalPadding);
      }
  }
  ...
  private int getHVerticalPadding() {
      if(cropHeight == 0){
          ...
          return (getHeight() - (cropHeight - 2 * horizontalPadding)) / 2;
      }
  }
  ...
  public void setClipImageView(ClipImageBorderView clipImageView) {
    this.mImageBorderView = clipImageView;
  }
}
