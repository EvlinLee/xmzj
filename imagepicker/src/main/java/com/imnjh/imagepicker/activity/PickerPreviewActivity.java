...
import com.imnjh.imagepicker.util.PhotoManager;
...
public class PickerPreviewActivity extends BasePickerActivity implements PickerAction {
  ...
  public static void startPicturePreviewFromPicker(Activity activityContext,
    ...
    PhotoManager.getInstance().mIntent=intent;
    Intent intent2 = new Intent(activityContext, PickerPreviewActivity.class);
    activityContext.startActivityForResult(intent2, requestCode);
  }
  ...
  private void initUI() {
    ...
    max = PhotoManager.getInstance().mIntent.getIntExtra(KEY_MAX_COUNT, 9);
    rowCount = PhotoManager.getInstance().mIntent.getIntExtra(KEY_ROW_COUNT, 4);
    fileChooseInterceptor = PhotoManager.getInstance().mIntent.getParcelableExtra(KEY_FILE_CHOOSE_INTERCEPTOR);
    ArrayList<Uri> uriParam = PhotoManager.getInstance().mIntent.getParcelableArrayListExtra(KEY_URIS);
    ArrayList<String> selectedParam = PhotoManager.getInstance().mIntent.getStringArrayListExtra(KEY_SELECTED);
    selectOriginal = PhotoManager.getInstance().mIntent.getBooleanExtra(KEY_SELECTED_ORIGINAL, false);
    initPosition = PhotoManager.getInstance().mIntent.getIntExtra(KEY_CURRENT_POSITION, 0);
    ...
    updateTitle();
    int pickTextRes = PhotoManager.getInstance().mIntent.getIntExtra(PARAM_CUSTOM_PICK_TEXT_RES, 0);
  ...
  private void startEnterAnimation(Uri enterImgUri) {
    hideMenu();
    final AnchorInfo anchorInfo = PhotoManager.getInstance().mIntent.getParcelableExtra(KEY_ANCHOR_INFO);
