...
public abstract class BasePopupWindow implements PopupWindow.OnDismissListener,
    ...
    private boolean isFull = true;                //是否全屏
    ...
    public BasePopupWindow(Activity activity, int resId, boolean outSide) {
        ...
        pWindow = new PopupWindow(parentView, WindowManager.LayoutParams.MATCH_PARENT,
                WindowManager.LayoutParams.MATCH_PARENT, outSide);
        pWindow.setAnimationStyle(0);
        pWindow.setOnDismissListener(this);
        pWindow.setBackgroundDrawable(
                new BitmapDrawable(getActivity().getResources(), (Bitmap) null));
    ...
    public void showPop(View view, int gravity, int x, int y){
        pWindow.showAtLocation(view, gravity, 0, 0);
    }
    ...
    public void setFull(boolean full) {
        ...
        if(pWindow != null){
            if(isFull){
                pWindow.setWidth(WindowManager.LayoutParams.MATCH_PARENT);
                pWindow.setHeight(WindowManager.LayoutParams.MATCH_PARENT);
