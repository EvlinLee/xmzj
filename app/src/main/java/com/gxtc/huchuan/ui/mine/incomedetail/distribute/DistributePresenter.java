...
public class DistributePresenter implements DistributeContract.Presenter{
    private ProfitContract.Source mData;
    private DistributeContract.View mView;

    private int start = 0;
    private String token = UserManager.getInstance().getToken();

    public DistributePresenter(DistributeContract.View mView) {
        this.mView = mView;
    ...
    public void getProfitList(String type,String dateType, final boolean isRefresh) {
        ...
        mData.getToatalProfitList(token,start+"",type, dateType,new ApiCallBack<List<DistributionBean>>() {
    ...
    public void getDistributionCount(String token, String type,String dataTpe) {
        mData.getCreateUserIncome(token, type,dataTpe, new ApiCallBack<DistributionCountBean>() {
            @Override
    ...
    @Override
    public void getDistributionIncomeSum(String token, String type, String dataTpe) {
        mData.getlistByDistributionBusinessSum(token,type,dataTpe, new ApiCallBack<DistributionCountBean>() {
            @Override
            public void onSuccess(DistributionCountBean data) {
                if(mView == null) return;
    ...
    public void getIncomeStatistics(HashMap<String, String> map) {
        //换成新的接口
        mData.getlistByDistributionBusiness(map, new ApiCallBack<List<DistributionBean>>() {
            @Override
            public void onSuccess(List<DistributionBean> data) {
                if(mView == null) return;
                if(data == null || data.size() == 0){
