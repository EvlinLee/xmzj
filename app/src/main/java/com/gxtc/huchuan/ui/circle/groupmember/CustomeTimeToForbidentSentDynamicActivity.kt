...
/**
 * Created by zzg on 2018/1/30  定时禁发动态
 */

class CustomeTimeToForbidentSentDynamicActivity : BaseTitleActivity(),View.OnClickListener,TimePickerView.OnTimeSelectListener {
    ...
    companion object {
        @JvmStatic
        fun jumpToCustomeTimeToForbidentSentDynamicActivity(context: Activity, groupChatId:String,groupId:Int) {
            val intent = Intent(context,CustomeTimeToForbidentSentDynamicActivity::class.java)
            intent.putExtra("groupChatId",groupChatId)
            intent.putExtra("groupId",groupId)
            context.startActivityForResult(intent,10006)
        }
    }
    ...
    override fun initView() {
        ...
        baseHeadView.showTitle("定时禁发动态").showBackButton {
            finish()
    ...
    fun setGroupTaskInTiming(){
        ...
        map.put("type","2") // 1 聊天室   2 定时禁发  3 全体禁发
        ...
        val sub = CircleApi.getInstance().setGroupTaskInTiming(map)
                ...
                .subscribe(ApiObserver<ApiResponseBean<Any>>(object : ApiCallBack<Any>(){
                    override fun onSuccess(data: Any?) {
                        ...
                        ToastUtil.showShort(MyApplication.getInstance(),"定时禁发动态成功")
                        val intent = Intent()
                        intent.putExtra("startTimeL",startTime.toString())
                        intent.putExtra("endTimeL",endTime.toString())
                        setResult(Activity.RESULT_OK,intent)
                        this@CustomeTimeToForbidentSentDynamicActivity.finish()
                    }
    ...
    private fun showTimePop() {
        ...
        timePickerView?.findViewById(R.id.second)?.visibility = View.GONE
        timePickerView?.show()
    }
