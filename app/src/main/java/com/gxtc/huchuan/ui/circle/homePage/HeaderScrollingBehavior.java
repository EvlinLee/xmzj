...
public class HeaderScrollingBehavior extends CoordinatorLayout.Behavior<RecyclerView> {
    ...
    public boolean layoutDependsOn(CoordinatorLayout parent, RecyclerView child, View dependency) {
        if (dependency != null && dependency.getId() == R.id.circle_header) {
    ...
    public boolean onDependentViewChanged(CoordinatorLayout parent, RecyclerView child, View dependency) {
        final float temp = Math.abs(dependency.getTranslationY() / dependency.getHeight());
        final float progress = 1.f - temp;
    ...
    private float getDependentViewCollapsedHeight() {
        return getDependentView().getResources().getDimension(R.dimen.head_view_hight);
    }
