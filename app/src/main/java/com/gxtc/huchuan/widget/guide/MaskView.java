...
class MaskView extends ViewGroup {
  ...
  @Override protected void onLayout(boolean changed, int l, int t, int r, int b) {
    ...
    if(mRectFList.size() != 0){
      mTargetRect.set(mRectFList.get(0));
    }
    for (int i = 0; i < count; i++) {
      child = getChildAt(i);
      if (child == null) {
  ...
  private void drawRoundRect(){
    if(mRectFList != null && mRectFList.size() > 0){
      for(RectF rectF: mRectFList){
  ...
  private void drawCircle(){
    if(mRectFList != null && mRectFList.size() > 0){
      for(RectF rectF: mRectFList){
        mEraserCanvas.drawCircle(rectF.centerX(), rectF.centerY(),rectF.width() / 2, mEraser);
      }
    }else{
  ...
  public void setRectFList(List<Rect> rects) {
    for(Rect rect: rects){
      mRectFList.add(new RectF(rect));
    }
    resetOutPaths();
    invalidate();
  }
