...
public class DividerItemDecoration extends RecyclerView.ItemDecoration {
     ...
     * @param dividerHeight 分割线高度
     * @param dividerColor  分割线颜色
     * @param headCount     头部视图的数量
     */
    public DividerItemDecoration(Context context, int orientation,
                                 int dividerHeight, int dividerColor,int headCount) {
    ...
    private void drawHorizontal(Canvas canvas, RecyclerView parent) {
        ...
        for (int i = 0; i < childSize; i++) {
            if(parent.getAdapter() instanceof LoadMoreWrapper){
                LoadMoreWrapper adapter = (LoadMoreWrapper) parent.getAdapter();
                if(adapter.hasLoadMore()){
                    int cout = adapter.getItemCount();
                    if(i == (cout - 1)){
                        return ;
                    }
                }
