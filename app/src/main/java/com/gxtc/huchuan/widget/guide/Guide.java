...
public class Guide implements View.OnKeyListener, View.OnClickListener {
  ...
  private MaskView onCreateView(Activity activity) {
    ...
    } else if(mConfiguration.mTargetViews != null && mConfiguration.mTargetViews.size() >= 0) {
      List<Rect> list = new ArrayList<>();
      for(View view: mConfiguration.mTargetViews){
        list.add(Common.getViewAbsRect(view, parentX, parentY));
      }
      maskView.setRectFList(list);
