...
public class PullUpToLoadMore extends ViewGroup {
    ...
    Scroller scroller = new Scroller(getContext(),new DecelerateInterpolator(0.9f));
    ...
    int speed = 200;
    int height = 0;
    int topScrollHeight = 0;        //第一个scrollview的高度
    ...
    private void init() {
        ...
        post(new Runnable() {
            ...
            public void run() {
                ...
                topScrollView.getChildAt(0).getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
                    @Override
                    public void onGlobalLayout() {
                        topScrollHeight = topScrollView.getHeight();
                        topScrollView.getChildAt(0).getViewTreeObserver().removeOnGlobalLayoutListener(this);
                        isIntercept = height > topScrollHeight;
                        topScrollViewIsBottom = height > topScrollHeight;
                    }
                });
            }
        });
