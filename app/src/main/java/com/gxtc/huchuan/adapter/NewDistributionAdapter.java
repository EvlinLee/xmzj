...
public class NewDistributionAdapter extends BaseRecyclerAdapter<DistributionBean> {
    public View.OnClickListener OnClickClassLayoutListenr;
    public View.OnClickListener OnClickClassBottomLayoutListenr;

    public NewDistributionAdapter(Context context, List<DistributionBean> list, int itemLayoutId) {
...
//                .setText(R.id.tv_commission,"佣金：￥"+distributionBean.getCommission())
                ...
                .setText(R.id.tv_buy_count,TextUtils.isEmpty(distributionBean.getJoinCount()) ? "0" : distributionBean.getJoinCount())
                .setText(R.id.tv_commission_count,TextUtils.isEmpty(distributionBean.getCommission()) ? "0" : distributionBean.getCommission());

        SpannableStringBuilder s = new SpannableStringBuilder("￥"+(Double.parseDouble(distributionBean.getFee()) * Double.parseDouble( distributionBean.getPent()) / 100));
