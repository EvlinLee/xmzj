...
import com.gxtc.huchuan.customemoji.adapter.HorizontalRecyclerviewAdapter;
import com.gxtc.huchuan.customemoji.adapter.NoHorizontalScrollerVPAdapter;
import com.gxtc.huchuan.customemoji.emotionkeyboardview.EmotionKeyboard;
import com.gxtc.huchuan.customemoji.emotionkeyboardview.NoHorizontalScrollerViewPager;
import com.gxtc.huchuan.customemoji.model.ImageModel;
...
import com.gxtc.huchuan.customemoji.utils.SharedPreferencedUtils;
...
public class EmotionMainFragment extends BaseFragment {
    ...
    public static final String BIND_TO_EDITTEXT="bind_to_edittext";                     //是否绑定当前Bar的编辑框的flag
    public static final String HIDE_BAR_EDITTEXT_AND_BTN="hide bar's editText and btn"; //是否隐藏bar上的编辑框和发生按钮
    public static final String IS_SHOW_EMOTION_BAR = "is_show_emotion_bar";             //是否展示顶部按钮tab
    private static final String CURRENT_POSITION_FLAG="CURRENT_POSITION_FLAG";          //当前被选中底部tab
    ...
    private View emotionButton;         //设置关联emotion 开关按钮

    //不可横向滚动的ViewPager
    private NoHorizontalScrollerViewPager viewPager;

    private boolean isBindToBarEditText = true;               //是否绑定当前Bar的编辑框,默认true,即绑定。  false,则表示绑定contentView,此时外部提供的contentView必定也是EditText
    private boolean isHidenBarEditTextAndBtn = false;         //是否隐藏bar上的编辑框和发生按钮,默认不隐藏
    private boolean isShowEmotionBar = true;                  //是否显示表情顶部bar
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        isBindToBarEditText = args.getBoolean(EmotionMainFragment.BIND_TO_EDITTEXT, false);
        isShowEmotionBar = args.getBoolean(EmotionMainFragment.IS_SHOW_EMOTION_BAR, true);

        initView(rootView);
        WeakReference<Activity> weakReferenceAcvity = new WeakReference<Activity>(getActivity());
        if(weakReferenceAcvity .get() == null) return null;
        mEmotionKeyboard = EmotionKeyboard.with(weakReferenceAcvity .get())
                ...
                .bindToEmotionButton(emotionButton == null ? rootView.findViewById(R.id.emotion_button): emotionButton)                  //绑定表情按钮
                .build();
        initListener();
        initDatas();
        //创建全局监听
    ...
    public void bindToEmotionButton(View emotionButton){
    ...
    protected void initView(View rootView){
        ...
        View emotion_view = rootView.findViewById(R.id.include_emotion_view);
        ...
        emotion_view.setVisibility(isShowEmotionBar ? View.VISIBLE : View.GONE);
    }
    ...
    protected void initDatas(){
        ...
        recyclerview_horizontal.setVisibility(list.size() == 1 ? View.GONE : View.VISIBLE);
